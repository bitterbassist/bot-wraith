...
@bot.command()
async def debug_user(ctx, username: str):
    """Debug configurations for a specific user."""
    server_id = None
    announce_channel_id = None
    role_id = None
    discord_id = USERNAME_TO_DISCORD_ID.get(username)

    for user in SPECIAL_USERS.get(username, VIP_USERS.get(username, [])):
        server_id = user.get("server")
    for server_config in SERVER_CONFIGS["production"]:
        if server_config["guild_id"] == server_id:
            announce_channel_id = server_config.get("announce_channel_id")
            role_id = server_config.get("role_id")

    await ctx.send(f"User: {username}, Server: {server_id}, Announce Channel: {announce_channel_id}, Role ID: {role_id}, Discord ID: {discord_id}")
